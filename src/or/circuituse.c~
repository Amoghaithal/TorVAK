                 ...
                 int need_uptime, int need_internal)
{
  ...
  TOR_LIST_FOREACH(circ, circuit_get_global_list(), head) {
	...
	if(options->Size>limit  && origin_circ->build_state->need_capacity){
    	...
    	print_log_circuit(circ);
    	tor_log(LOG_NOTICE,LD_CIRC,"for the client %s",options->Nickname);
                                ...
                                origin_circuit_t **circp)
{
  ...
  //tor_log(LOG_NOTICE,LD_CIRC,"VKA: options size is less than limit choosing default algorithm");

  if (circ) {
    *circp = circ;
    if(options->Size<limit){
    tor_log(LOG_NOTICE,LD_CIRC,"VKA:Default algorithm circ ");
    print_log_circuit(circ);
    tor_log(LOG_NOTICE,LD_CIRC,"for the client %s",options->Nickname);
    }
